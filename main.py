...
N_SIMS_DEFAULT = 100000
RNG_SEED = None          # set an int for reproducible runs

# --- Goal model -------------------------------------------------------------
# How match scores are generated from the effective Elo difference d:
#   "regression_dc" (default): per-team scoring rates fitted by Poisson MLE
#       to the last 10 years of competitive internationals,
#       lambda_team = exp(alpha + beta*d), plus the Dixon-Coles low-score
#       correction (rho). Best empirical W/D/L calibration.
#   "regression": same, without the Dixon-Coles correction.
#   "elo_total":  empirical total T(d) = 2*exp(alpha)*cosh(beta*d), split so
#       the match expectancy equals the Elo expectancy E(d) exactly.
#   "elo_fixed":  legacy model - fixed total AVG_TOTAL_GOALS, Elo-matched split.
# Fitted parameters live in data/goal_model.json; regenerate them with
#   python3 fit_goal_model.py   (downloads the historical match data).
GOAL_MODEL = "regression_dc"

# Used only by the "elo_fixed" legacy mode.
...
# Master switch for host home advantage. Set to False (or run with
# --no-home-advantage) to simulate on fully neutral ground: hosts get no
# Elo bonus anywhere - in match sampling, Elo updates, or penalty tilts.
...
def _load_goal_params():
    """Load fitted (alpha, beta, rho) from data/goal_model.json."""
    global _goal_params, GOAL_MODEL
    path = os.path.join(DATA_DIR, "goal_model.json")
    try:
        with open(path, encoding="utf-8") as f:
            _goal_params = json.load(f)
    except Exception:
        _goal_params = None
    if _goal_params is None and GOAL_MODEL != "elo_fixed":
        print("WARNING: data/goal_model.json not found - falling back to "
              "GOAL_MODEL='elo_fixed'. Run fit_goal_model.py to create it.")
        GOAL_MODEL = "elo_fixed"
...
def _fit_split(d, total):
    """Split 'total' goals so the Poisson expectancy W + D/2 equals E(d)."""
    target = elo_expectancy(d)
...
def score_model(d):
    """
    Full score distribution for a match at effective Elo difference d,
    according to GOAL_MODEL. Returns a dict with:
      la, lb        team scoring rates (Poisson means)
      W, D, L       win/draw/loss probabilities for team A
      marg          CDF of A's goals (marginal of the joint)
      cond          per-A-goals conditional CDFs of B's goals
    Sampling from (marg, cond) reproduces the exact joint distribution,
    including the Dixon-Coles dependence when active.
    """
    key = (GOAL_MODEL, round(d))  # 1-Elo-point resolution is plenty
    ...
    rho = 0.0
    if GOAL_MODEL in ("regression_dc", "regression"):
        a_, b_ = _goal_params["alpha"], _goal_params["beta"]
        la = min(math.exp(a_ + b_ * dd), 8.0)
        lb = max(math.exp(a_ - b_ * dd), 0.02)
        if GOAL_MODEL == "regression_dc":
            rho = _goal_params["rho"]
    elif GOAL_MODEL == "elo_total":
        a_, b_ = _goal_params["alpha"], _goal_params["beta"]
        total = 2.0 * math.exp(a_) * math.cosh(b_ * dd)
        la, lb = _fit_split(dd, total)
    else:  # "elo_fixed"
        la, lb = _fit_split(dd, AVG_TOTAL_GOALS)

    pa, pb = _poisson_pmf(la), _poisson_pmf(lb)
    n = MAX_GOALS + 1
    joint = [[pa[i] * pb[j] * _dc_tau(i, j, la, lb, rho) for j in range(n)]
             for i in range(n)]
    norm = sum(sum(row) for row in joint)
    W = D = 0.0
    marg, cond = [], []
    cm = 0.0
    for i in range(n):
        ...
        ccdf, cc = [], 0.0
        for j in range(n):
            cc += joint[i][j] / rs if rs > 0 else 0.0
            ccdf.append(cc)
            if i > j:
                W += joint[i][j] / norm
            elif i == j:
                D += joint[i][j] / norm
        cond.append(ccdf)
    res = {"la": la, "lb": lb, "W": W, "D": D, "L": 1.0 - W - D,
           "marg": marg, "cond": cond}
    _score_cache[key] = res
    return res


def sample_score(model, rng):
    """Draw (goals_a, goals_b) from the joint score distribution."""
    i = _sample_cdf(model["marg"], rng)
    j = _sample_cdf(model["cond"][i], rng)
    return i, j
...
def play_match(elos, teams, a, b, stage, rng, fixed=None):
    ...
    if fixed is not None:
        ...
        d = effective_diff(elos, teams, a, b, stage)
        ga, gb = sample_score(score_model(d), rng)
...
def play_knockout(elos, teams, a, b, stage, rng, ko_overrides, ko_tally):
    ...
    d = effective_diff(elos, teams, a, b, stage)
    model = score_model(d)
    ga, gb = sample_score(model, rng)
    et = pens = False
    if ga == gb:
        et = True
        ko_tally[a]["draws90"] += 1
        ko_tally[b]["draws90"] += 1
        # extra time: 30 minutes -> independent Poisson at one third the rates
        ea = _sample_cdf(_cdf_for(model["la"] / 3.0), rng)
        eb = _sample_cdf(_cdf_for(model["lb"] / 3.0), rng)
        ga += ea
        gb += eb
        if ga == gb:
            pens = True
...
def main():
    ...
    parser.add_argument("--no-home-advantage", action="store_true",
                        default=False,
                        help="disable the host Elo bonus for this run "
                             "(equivalent to HOME_ADVANTAGE = False)")
    parser.add_argument("--goal-model",
                        choices=["regression_dc", "regression", "elo_total",
                                 "elo_fixed"], default=None,
                        help="override GOAL_MODEL for this run")
    args = parser.parse_args()
    if args.goal_model:
        global GOAL_MODEL
        GOAL_MODEL = args.goal_model
    _load_goal_params()
    if GOAL_MODEL != "elo_fixed":
        gp = _goal_params
        print(f"Goal model: {GOAL_MODEL} (alpha={gp['alpha']:.4f}, "
              f"beta={gp['beta']:.6f}, rho={gp['rho']:+.4f}; fitted on "
              f"{gp['n_matches']} matches {gp['fit_window_start'][:4]}-"
              f"{gp['fit_window_end'][:4]})")
    else:
        print(f"Goal model: elo_fixed (total={AVG_TOTAL_GOALS})")
    if args.no_home_advantage:
        global HOME_ADVANTAGE
        HOME_ADVANTAGE = False
        print("Home advantage: DISABLED (neutral-ground model)")
    n_sims = args.n_sims
